...
response = client.responses.create(
    ...
    input=[{"role": "user", "content": "What's the price of AAPL?"}],
    tools=tools
)

for item in response.output:
    if item.type == "function_call":
        result = get_stock_price(json.loads(item.arguments)["ticker"])
...
def generate(prompt):
    ...
    if tool_call:
        result = get_stock_price(json.loads(tool_call.arguments)["ticker"])
...
return StreamingResponse(generate("What's the price of AAPL?"))
```
